...
class Timestream(_Datastruct_base):
    ...
    **Attribute and Method Listings**
    ...
    def __init__(self, path, files_in=None, mc=0, xf=None, cr=None, version='2026.dev.0', store_copy=False, raisenotel=False, mode='auto',impose_frame='J2000_soft',lon=lon12m,lat=lat12m,height=height12m,telname='ARO 12m',instname='TIME',skip_last=True):
        ...
        if mode == 'raw':
            ...
            if version in ['2024.dev.1','2026.dev.0']:
                ...
                hk_path = base_path.parents[0] / f'hk_netcdf_files/hk_{run_id}'  
                mce_path = base_path.parents[0] / f'netcdf_files/mce_{run_id}' 
            else: 
                hk_path = None 
                mce_path = path 
    ...
    def remove_common_mode_noise(self,save_path,pixel,debugging_plots=True,common_mode_params={'remove_cm':True,'strength':1, 'mux_block':4,'freq_block':[0,8,16,24,36,48,60]},**kwargs):
        """
        ...
        self.pair_wise_correlation(savepath=cm_savepath,stagename='before')
        #normalize the tod by their std deviation
        tod = self.build_full_xf_tod() 
        tod_norm,tod_norm_factor = self.normalize_tod(tod,strength=strength_param,return_norms=True)
        #print('tod norm',tod_norm.shape,np.any(np.isfinite(tod_norm)))

        #mux column template subtraction 
        mux_subtracted_tod_norm, mux_template,scaling_factors_mux = self.mux_template_wrapper(tod = tod_norm ,pixel=pixel, MUXC_BLOCK=mux_block_param,cm_savepath=cm_savepath,debugging_plots=debugging_plots,ts_savepath=ts_savepath)
        #print('mux_subtracted_tod_norm',mux_subtracted_tod_norm.shape,np.sum(np.isfinite(mux_subtracted_tod_norm))) 
        #print('mux_template examples',mux_template)

        #debugging for nan issues 
        #check if any mux_template is all nans 
        #for i, template in enumerate(mux_template):
        #    if np.all(~np.isfinite(template)):
        #        print(f'Mux template {i} is all nans')
        #check if any scaling factor is nan:
        #for i, scaling in enumerate(scaling_factors_mux):
        #    if not np.isfinite(scaling):
        #        print(f'Scaling factor for detector {i} is nan')
        



        xf_good = np.asarray(self.header['xf_coords'])
        if xf_good.ndim == 1:
            xf_good = np.vstack(xf_good)
        xf_good = xf_good.astype(int)

        #for i, (x,f) in enumerate(xf_good):
            # if debugging_plots: 
                # fig, ax = plt.subplots(4,1,figsize=(10,10),sharex=True,sharey=True)
                # ax[0].plot(tod_norm[x + 16*f],label='Normalized TOD',alpha=0.7)
                # ax[1].plot(mux_subtracted_tod_norm[x + 16*f],label='Mux CM Subtracted TOD',alpha=0.7)
                # ax[0].set_title(f'Detector x={x}, f={f} Mux CM Subtraction')
                # ax[0].legend()
                # ax[1].legend()
                # ax[2].plot(tod_norm[x + 16*f]- mux_subtracted_tod_norm[x + 16*f],label='Removed Mux CM Template',color='red',alpha=0.7)
                # ax[2].set_title('Removed Mux CM Template')
                # ax[2].legend()

                # muxc_idx = coordinates.xf_to_muxcr(x,f)[0] // 4 #debug mux block 4
                # ax[3].plot(mux_template[muxc_idx],label=f'Mux CM Template Block {muxc_idx} with scaling{scaling_factors_mux[x+16*f]}',color='green',alpha=0.7)
                # ax[3].legend()
                # #ax[].legend()
                ...
                # #plt.xlim(9000,12000)
                # plt.tight_layout()
                # plt.savefig(os.path.join(ts_savepath,f'detector_x{x}_f{f}_mux_cm_subtraction.png'))
                # plt.close()

        #normalize again 
        tod_norm2,tod_norm_factor2 = self.normalize_tod(mux_subtracted_tod_norm,strength = strength_param, return_norms=True)
        #print('tod_norm2 ',tod_norm2.shape,np.any(np.isfinite(tod_norm2)))
        #print('tod_norm_factor2 ',tod_norm_factor2.shape,np.any(np.isfinite(tod_norm_factor2)))
      

        #frequency block template subtraction
        freq_subtracted_tod_norm, freq_template, scaling_factors_freq= self.freq_template_wrapper(tod = tod_norm2,pixel=pixel, FREQ_BLOCK=freq_block_param,cm_savepath=cm_savepath,debugging_plots=debugging_plots)
        
        #print('freq_subtracted_tod_norm',freq_subtracted_tod_norm.shape,np.any(np.isfinite(freq_subtracted_tod_norm))) 
        #print('freq_template examples',freq_template)

        #unnormalize the timestreams
        tod_after =  freq_subtracted_tod_norm * tod_norm_factor[:,None] * tod_norm_factor2[:,None]
        #print('tod_norm_factor ',np.array(tod_norm_factor).shape,np.any(np.isfinite(np.array(tod_norm_factor))))
        #print('tod_after ',tod_after.shape,np.any(np.isfinite(tod_after)))

        for i, (x,f) in enumerate(xf_good):
            #self.data_copy[i] = freq_subtracted_tod[x + 16*f]
             
            if debugging_plots: 
                # fig, ax = plt.subplots(4,1,figsize=(10,10),sharex=True,sharey=True)
                # ax[0].plot(tod_norm[x + 16*f],label='Normalized TOD',alpha=0.7)
                # ax[1].plot(freq_subtracted_tod_norm[x + 16*f],label='Frequency Subtracted TOD',alpha=0.7)
                # ax[0].set_title(f'Detector x={x}, f={f} Frequency CM Subtraction')
                # ax[0].legend()
                # ax[1].legend()
                # ax[2].plot(tod_norm[x + 16*f]- freq_subtracted_tod_norm[x + 16*f],label='Removed Frequency CM Template',color='red',alpha=0.7)
                # ax[2].set_title('Removed frequency  CM Template')
                # ax[2].legend()

                # FREQ_BLOCK = [0,8,16,24,36,48,60]
                # for i in range(len(FREQ_BLOCK)-1):
                #     if FREQ_BLOCK[i] <= f < FREQ_BLOCK[i+1]:
                #         freq_idx = i
                #         break

                # ax[3].plot(freq_template[freq_idx],label=f'Frequency CM Template Block {freq_idx} with scaling{scaling_factors_freq[x+16*f]}',color='green',alpha=0.7)
                # ax[3].legend()
                # #ax[].legend()
                # ax[-1].set_xlabel("Samples")
                ...
                # #plt.xlim(9000,12000)
                # plt.tight_layout()
                # plt.savefig(os.path.join(ts_savepath,f'detector_x{x}_f{f}_freq_cm_subtraction.png'))
                # plt.close()

                fig,ax = plt.subplots(3,1,figsize=(10,6))
                ax[1].plot(tod_after[x + 16*f],label='Final TOD after CM Subtraction',alpha=0.7)
                ax[0].plot(tod[x + 16*f],label='Original TOD',alpha=0.7)
                ax[0].set_title(f'Detector x={x}, f={f} ')
                ax[2].plot(tod[x + 16*f]- tod_after[x + 16*f],label='Total Removed CM Template',color='purple',alpha=0.7)
                ax[0].legend()
                ax[1].legend()
                ax[2].legend()
                plt.savefig(os.path.join(ts_savepath,f'detector_x{x}_f{f}_final_tod_comparison.png'))
                plt.close()

            self.data[i] = tod_after[x + 16*f] 
        #print(self.data.shape,np.all(np.isfinite(self.data)))
        self.pair_wise_correlation(savepath=cm_savepath,stagename='after')
    

    def freq_template_wrapper(self,tod,pixel,FREQ_BLOCK=[0,8,16,24,36,48,60],cm_savepath=None,debugging_plots=True):
        """
        wrapper function to build and remove the frequency common mode template
        """
        template_per_freq,detector_to_freqtemplate= self.make_freq_block_templates(tod=tod,FREQ_BLOCK=FREQ_BLOCK,cm_savepath=cm_savepath)
        
        if debugging_plots:
            plotting_array = np.full((16,60),np.nan)
            for (x,f) in self.header['xf_coords']:
                plotting_array[x,f] = detector_to_freqtemplate[(x,f)]
            ...
            im = ax.imshow(plotting_array, origin='lower', aspect='auto', cmap =cmap)

            ax.set_xticks(np.arange(-0.5, 60, 1), minor=True)
            ax.set_yticks(np.arange(-0.5, 16, 1), minor=True)
            ax.grid(which='minor', color='black', linestyle='-', linewidth=0.5)

            cbar = plt.colorbar(im, ax=ax, ticks=np.arange(0,len(FREQ_BLOCK)))
        ...
        self.plot_commonmode_template(templates = template_per_freq, pixel = pixel, savepath = cm_savepath, template_kind = 'freq')
        freq_correlcoefs = self.correlate_detectors_to_freq_template(tod = tod, template_per_freq = template_per_freq, FREQ_BLOCK=FREQ_BLOCK)
        #print('freq correlcoefs',freq_correlcoefs)
        if debugging_plots:
            fig, ax = plt.subplots(figsize=(8,6))
            ax.hist(freq_correlcoefs,bins=30,alpha=0.7,color='blue')
            ax.set_xlabel('Correlation Coefficient to Frequency CM Template')
            ax.set_ylabel('Number of Detectors')
            #ax.set_title('Histogram of Correlation Coefficients to Frequency CM Template')
            plt.tight_layout()
            plt.savefig(os.path.join(cm_savepath,'freq_cm_correlation_coefficients_histogram.png'))
            plt.close()

        scaling_factors_freq = self.calculate_scaling_for_all(rs=freq_correlcoefs)
        freq_subtracted_tod_norm = self.subtract_freq_template(tod = tod,template_per_freq = template_per_freq, scaling_factors = scaling_factors_freq, FREQ_BLOCK=FREQ_BLOCK)
        return freq_subtracted_tod_norm, np.array(template_per_freq),scaling_factors_freq

    def mux_template_wrapper(self,tod,pixel,MUXC_BLOCK=1,cm_savepath=None,debugging_plots=True,ts_savepath=None):
        """
        wrapper function to build and remove the mux common mode template
        """
        template_per_muxc, detector_to_muxtemplate = self.make_muxc_block_template(tod = tod , MUXC_BLOCK=MUXC_BLOCK,cm_savepath=cm_savepath)

        if debugging_plots: 
            #plot for each good xf which template it corresponds to
            plotting_array = np.full((16,60),np.nan)
            for (x,f), muxc_idx in detector_to_muxtemplate.items():
                plotting_array[x,f] = muxc_idx


            colors32 = ['#1f77b4', '#669fce', '#aec7e8', '#ff7f0e', '#ff9d43', '#ffbb78',
            ...
            colors_plot = colors32[:len(template_per_muxc)]
            ...
            ax.set_yticks(np.arange(-0.5, 16, 1), minor=True)
            ax.grid(which='minor', color='black', linestyle='-', linewidth=0.5)
            cbar = plt.colorbar(im, ax=ax, ticks=np.arange(0,len(template_per_muxc)))
        ...
        #print('template shape',np.array(template_per_muxc).shape)
        self.plot_commonmode_template(templates = template_per_muxc, pixel = pixel, savepath = cm_savepath, template_kind = 'mux')
        mux_correlcoefs = self.correlate_detectors_to_mux_template(tod = tod, template_per_muxc = template_per_muxc, MUXC_BLOCK=MUXC_BLOCK) 
        
        #print('mux correlcoefs',mux_correlcoefs)
        
        if debugging_plots:
            fig, ax = plt.subplots(figsize=(8,6))
            ax.hist(mux_correlcoefs,bins=30,alpha=0.7,color='green')
            ax.set_xlabel('Correlation Coefficient to Mux CM Template')
            ax.set_ylabel('Number of Detectors')
            #ax.set_title('Histogram of Correlation Coefficients to Mux CM Template')
            plt.tight_layout()
            plt.savefig(os.path.join(cm_savepath,'mux_cm_correlation_coefficients_histogram.png'))
            plt.close() 

        print(np.sum(np.isfinite(mux_correlcoefs)), 'out of', len(mux_correlcoefs), 'detectors have finite correlation coefficients to the mux template')
        xf_good = np.asarray(self.header['xf_coords'])
        ...
        print(xf_good.shape, 'good detectors in total')
        for f in range(60):
            for x in range(16):
                if (x,f) in xf_good: 
                    idx = x+16*f #np.where((xf_good[:,0] == x) & (xf_good[:,1] == f))[0][0]
                    if not np.isfinite(mux_correlcoefs[idx]):
                        print(f'Detector x={x}, f={f} has non-finite correlation coefficient to mux template: {mux_correlcoefs[idx]}')

        scaling_factors_mux = self.calculate_scaling_for_all(rs=mux_correlcoefs)
        mux_subtracted_tod_norm = self.subtract_muxc_template(tod = tod, template_per_muxc = template_per_muxc, scaling_factors = scaling_factors_mux, MUXC_BLOCK=MUXC_BLOCK,ts_savepath=ts_savepath)
        return mux_subtracted_tod_norm,np.array(template_per_muxc),scaling_factors_mux

    def make_freq_block_templates(self,tod,FREQ_BLOCK = [0,8,16,24,36,48,60],save_detector_to_template = True, cm_savepath=None):
        """
        FREQ_BLOCK: this is DIFFERENT from the MUXC_BLOCK! 
        these are the dividing lines for different frequency blocks so you could have subarrays. the default is actually to have 6 templates
        where each correspond to a subarray. 
        """

        time = self.t
        xf, xf_fxsort = self.get_and_sort_xf(fx_sort=True)
        SPECTRAL_CHANNELS = (19,22,23,24,40)  

        template_per_freq = []
        for block_i in range(len(FREQ_BLOCK)-1): 
            collected = []  
            f_start = FREQ_BLOCK[block_i]#ci * FREQ_BLOCK
            f_stop = FREQ_BLOCK[block_i+1]#(ci + 1) * FREQ_BLOCK
            for (x, f) in xf:
                if (f >= f_start) and (f < f_stop) and (f not in SPECTRAL_CHANNELS):
                    index = x + 16*f
                    value = tod[index]
                    collected.append(value)
            median = np.nanmedian(collected, axis=0)
            template_per_freq.append(median)
        for block_i in range(len(FREQ_BLOCK)-1):
            if not len(template_per_freq[block_i].shape):
                template_per_freq[block_i] = np.zeros(len(time))

        #for each detector(x,f), store which frequency block template it corresponds to in a dictionary 
        detector_to_freqtemplate = {}
        for (x,f) in xf:
            for block_i in range(len(FREQ_BLOCK)-1):
                if FREQ_BLOCK[block_i] <= f < FREQ_BLOCK[block_i+1]:
                    detector_to_freqtemplate[(x,f)] = block_i
                    break
        
        if save_detector_to_template:
            np.save(os.path.join(cm_savepath,'detector_to_freqtemplate.npy'),detector_to_freqtemplate)

        return template_per_freq, detector_to_freqtemplate
    ...
    def scaling_factor(self,r,a=20,b=6,exponent=0.25):
        """
        so that the template is scaled depending on the sign of r 
        If a detector correspond to a known spectral line,it's likely that it will correlate less to 
        the common mode template (since they are not used to build it). 
        we want to raise the scaling base to an exponent such that we boosts moderate-small correlations
        ...
        exponent : Exponent to raise the scaling base to (default: 0.25).
        0.25 for boosting moderate-small correlations for detectors in spectral channels 
        1 for normal channels.
        """ 
        scaling_base = self.corrcoef_to_scaling(r,a,b)
        
        #if np.isnan(scaling_base): 
        #    print(f'Correlation coefficient: {r}, Scaling base before exponent: {scaling_base}')

        if exponent: 
            return np.sign(scaling_base)*np.abs(scaling_base) ** exponent 
        else:
            return np.sign(scaling_base)*np.abs(scaling_base)

    def subtract_muxc_template(self,template_per_muxc,scaling_factors,tod,MUXC_BLOCK=4,ts_savepath=None):
        xf_full = np.array([[x, f] for f in range(60) for x in range(16)]) 

        print('subtracting muxc template...')
        #print('tod',np.array(tod).shape)
        ...
        if xf_good.ndim == 1:
            xf_good = np.vstack(xf_good)
        xf_good = xf_good.astype(int)
        #print('xf_good',xf_good)

        for (x,f) in xf_full: 
            det_idx = x + 16*f
            muxc_idx = coordinates.xf_to_muxcr(x,f)[0] // MUXC_BLOCK
            scaling = scaling_factors[det_idx]
            template = template_per_muxc[muxc_idx]
            # if (x,f) in xf_good:
            #     fig,ax = plt.subplots(4,1,figsize=(10,6),sharey=True)
            #     ax[0].plot(tod[det_idx],label='Before Subtraction')
            #     ax[0].set_title(f'Before Subtraction')
            #     ax[1].plot(template,label='Original Template')
            #     ax[1].set_title(f'Original Template')
            #     ax[2].plot(scaling * template,label='Scaled Template')
            #     ax[2].set_title(f'Scaled Template')
            

            tod[det_idx] -= scaling * template
            # if (x,f) in xf_good:
            #     ax[3].plot(tod[det_idx],label='After Subtraction')
            #     ax[3].set_title(f'After Subtraction')
            #     plt.savefig(os.path.join(ts_savepath,'detector_x{}_f{}_muxc_block{}.png'.format(x,f,MUXC_BLOCK)))
            #     plt.close()
    ...
    def subtract_freq_template(self,template_per_freq,scaling_factors,tod,FREQ_BLOCK=[0,8,16,24,36,48,60]):
        print('subtracting freq template...')
        xf_full = np.array([[x, f] for f in range(60) for x in range(16)]) 
        xf_good = np.asarray(self.header['xf_coords'])
        ...
        for (x,f) in xf_full: 
            ...
            for i in range(len(FREQ_BLOCK)-1):
                if FREQ_BLOCK[i] <= f < FREQ_BLOCK[i+1]:
                    ...
                    break
            scaling = scaling_factors[det_idx]
            template = template_per_freq[freq_idx]
            # if (x,f) in xf_good:
            #     fig,ax = plt.subplots(4,1,figsize=(10,6),sharey=True)
            #     ax[0].plot(tod[det_idx],label='Before Subtraction')
            #     ax[0].set_title(f'Before Subtraction')
            ...
            #     ax[3].plot(tod[det_idx],label='After Subtraction')
            #     ax[3].set_title(f'After Subtraction')
            #     plt.savefig(os.path.join(ts_savepath,'detector_x{}_f{}_freq_block{}.png'.format(x,f,freq_idx)))
            #     plt.close()

        return tod

    def calculate_scaling_for_all(self,rs): 
        """
        rs: array of correlation coefficients for all detectors
        """
        ...
        all_scaling = np.zeros_like(rs) 

        for (x,f) in xf_full: 
            if (int(x), int(f)) in set(map(tuple, self.header['xf_coords'].tolist())):
                if f in SPECTRAL_CHANNELS: 
                    det_idx = x + 16*f
                    
                    all_scaling[det_idx] = self.scaling_factor(rs[det_idx],exponent=0.25)

                    #if not np.isfinite(all_scaling[det_idx]):
                    #    print(f'Detector x{x} f{f} has scaling factor of nan with r {rs[det_idx]}')
                    #    print(f'the corrcoef_to_scaling value is {self.corrcoef_to_scaling(rs[det_idx])}')

                else:
                    det_idx = x + 16*f
                    all_scaling[det_idx] = self.scaling_factor(rs[det_idx],exponent=1.0)
            else: 
                det_idx = x + 16*f
                all_scaling[det_idx] = 0.0 

        return all_scaling


    def correlate_detectors_to_mux_template(self,template_per_muxc,tod,MUXC_BLOCK=1):
        """
        correlate each detector timestream to the corresponding common mode template
        """
        time = self.t
        n_detectors = tod.shape[0]
        mux_correlcoefs_list = []

        N = len(time)
        which_detector_see_muxcm = np.empty((16,60))
        t_start =  0 #0 # N // 4
        t_stop = N #N #3 * N // 4
        for f in range(60):
            for x in range(16):
                muxc = coordinates.xf_to_muxcr(x, f)[0]
                bin_index = muxc // MUXC_BLOCK
                signal_trace = tod[x + 16*f, t_start:t_stop]
                template_trace = template_per_muxc[bin_index][t_start:t_stop]
                #why can the correlation coefficient be nan? it shouldn't be if the template and signal trace are not all nans.
                if (int(x), int(f)) in set(map(tuple, self.header['xf_coords'].tolist())):
                    print('signal_trace',np.any(np.isfinite(signal_trace)), 'template_trace',np.any(np.isfinite(template_trace)))
                corr_coef = np.corrcoef(signal_trace, template_trace)[1, 0] 
                if (int(x), int(f)) in set(map(tuple, self.header['xf_coords'].tolist())):
                    print(f'Detector x{x} f{f} has correlation coefficient {corr_coef} to mux template block {bin_index}')
                which_detector_see_muxcm[x,f] = corr_coef
                mux_correlcoefs_list.append(corr_coef)
        mux_correlcoefs = np.array(mux_correlcoefs_list)
        return mux_correlcoefs

    def correlate_detectors_to_freq_template(self,template_per_freq,tod,FREQ_BLOCK = [0,8,16,24,36,48,60]):
        """
        correlate each detector timestream to the corresponding common mode template
        """
        time = self.t
        n_detectors = tod.shape[0]
        freq_correlcoefs_list = []

        N = len(time)
        which_detector_see_freqcm = np.empty((16,60))
        t_start = 0 # N // 4
        t_stop = N #3 * N // 4
        for f in range(60):
            for x in range(16):
                for block_i in range(len(FREQ_BLOCK)-1):
                    f_start = FREQ_BLOCK[block_i]
                    f_stop = FREQ_BLOCK[block_i+1]
                    if (f >= f_start) and (f < f_stop):
                        signal_trace = tod[x + 16*f, t_start:t_stop]
                        template_trace = template_per_freq[block_i][t_start:t_stop]
                        corr_coef = np.corrcoef(signal_trace, template_trace)[1, 0]
                        which_detector_see_freqcm[x,f] = corr_coef
                        freq_correlcoefs_list.append(corr_coef)
        freq_correlcoefs = np.array(freq_correlcoefs_list)  
        return freq_correlcoefs

    def make_muxc_block_template(self,tod,MUXC_BLOCK=1,save_detector_to_template = True, cm_savepath=None):
        """
        Compute common mode template in blocks of mux columns. 
        make sure the blocks divide up the 32 mux columns evenly. 
        too little detectors in the same block you start picking up signals 
        too many detectors in the same block and you don't really pick up any meaningful common mode noise
        so your options are 
        1: do common mode per mux-column (this only works when you have a LOT of detectors)
        2: the intermediate ground. 
        4: common mode per 4 mux-columns (8 blocks). this is the largest block i would go to. (this is used for SagA which had 179 detectors)
        """
        ...
        #mux_retain_x = [[x for (x,f) in xf_fxsort if (coordinates.xf_to_muxcr(x,f)[0]>=ci*MUXC_BLOCK) and (coordinates.xf_to_muxcr(x,f)[0]<(ci+1)*MUXC_BLOCK) and f not in SPECTRAL_CHANNELS] for ci in range(32//MUXC_BLOCK)]

        template_per_muxc = []
        for ci in range(32 // MUXC_BLOCK):
            collected = []
            for (x, f) in xf_fxsort:
                muxc = coordinates.xf_to_muxcr(x, f)[0]
                if (muxc >= ci * MUXC_BLOCK) and (muxc < (ci + 1) * MUXC_BLOCK) and (f not in SPECTRAL_CHANNELS):
                    index = x + 16*f
                    value = tod[index]
                    collected.append(value)
            median = np.nanmedian(collected, axis=0)
            template_per_muxc.append(median)

        for ci in range(32//MUXC_BLOCK):
            if not len(template_per_muxc[ci].shape):
                template_per_muxc[ci] = np.zeros(len(time))

        # for each (x,f), store which mux block it belongs to in a dictionary 
        detector_to_muxtemplate = {}
        for (x,f) in xf_fxsort:
            muxc = coordinates.xf_to_muxcr(x, f)[0]
            mux_block = muxc // MUXC_BLOCK
            detector_to_muxtemplate[(x,f)] = mux_block 

        if save_detector_to_template:
            np.save(os.path.join(cm_savepath,'detector_to_muxtemplate.npy'),detector_to_muxtemplate)



        return template_per_muxc, detector_to_muxtemplate 
    

    def pair_wise_correlation(self, savepath, stagename='before'): 
        """
        before using the common mode template, we want to plot the detector pair-wise correlation matrix 
        to see in what structure does the correlation largely occur in. 

        stagename: str: to plot the correlation coefficient plots for "before" or "after" common mode removal.
        """

        tod = self.build_full_xf_tod()
        tod_norm = self.normalize_tod(tod,strength=10)

        xf, xf_fxsort, xf_rcsort = self.get_and_sort_xf(fx_sort=True,rc_sort=True)
        time = np.arange(tod.shape[-1])

        fig, axes = plt.subplots(1, 2, figsize=(15, 11), gridspec_kw={'wspace': 0.1})

        # ===== Second subplot (sorted by f and then x) =====
        ax = axes[1]
        im1 = ax.imshow(
            np.corrcoef(np.array([
                tod_norm[x + 16*f, len(time)//4 : 3*len(time)//4]
                for (x, f) in xf_fxsort
            ])),
            vmin=-1, vmax=1, cmap='RdBu', origin='lower'
        )

      
        subarray_grids = [8, 16, 24, 36, 48, 60]
        f_sorted = np.asarray(xf_fxsort)[:, 1].astype(int)

        for fb in subarray_grids:
            k = np.searchsorted(f_sorted, fb, side="left")  
            pos = k - 0.5 
           
            if 0 <= pos <= len(f_sorted) - 1:
                ax.axhline(pos, color="k", alpha=0.7)
                ax.axvline(pos, color="k", alpha=0.7)

        # Hide ticks
        ax.set_xticks([])
        ax.set_yticks([])

        # Add custom labels in the middle of each grid cell
        #subarray_grids = [8, 16, 24, 36, 48, 60]
        boundaries = [-0.5] + [n - 0.5 for n in subarray_grids] + [len(xf_fxsort) - 0.5]

        centers = [(boundaries[i] + boundaries[i+1]) / 2 for i in range(len(boundaries) - 1)]
        labels = [f"SUBARRAY {i}" for i in range(len(centers))]

        ax.set_xticks(centers)
        ax.set_xticklabels(labels, fontsize=12, ha='center')  #text centered on tick
        ax.tick_params(axis='x', pad=5) 

        ax.set_yticks(centers)
        ax.set_yticklabels(labels, rotation=90, fontsize=12, va='center')
        ax.tick_params(axis='y', pad=5)

        ax.text(
            0.98, 0.98, "sorted by f and then x",
            transform=ax.transAxes, ha='right', va='top',
            fontsize=18, fontweight='bold'
        )

        # ===== First subplot (sorted by mux-c and then mux-r) =====
        ax = axes[0]
        im2 = ax.imshow(
            np.corrcoef(np.array([
                tod_norm[x + 16*f, len(time)//4 : 3*len(time)//4]
                for (x, f) in xf_rcsort
            ])),
            vmin=-1, vmax=1, cmap='RdBu', origin='lower'
        )

        # Draw grid lines every 4 mux-c steps starting at c=4
        for c in range(1, 32)[3::4]:
            ax.axhline(
                max([i + 0.5 for i, (x, f) in enumerate(xf_rcsort)
                    if coordinates.xf_to_muxcr(x, f)[0] < c]),
                color='k', alpha=0.7
            )
            ax.axvline(
                max([i + 0.5 for i, (x, f) in enumerate(xf_rcsort)
        ...
        # Custom labels based on mux-c groups
        boundaries = [0] + [
            max([i + 0.5 for i, (x, f) in enumerate(xf_rcsort)
                if coordinates.xf_to_muxcr(x, f)[0] < c])
            for c in range(1, 32)[3::4]] + [len(xf_rcsort)]
        centers = [(boundaries[i] + boundaries[i+1]) / 2 for i in range(len(boundaries) - 1)]
        labels = ['MUX-c BLOCK' + str(i) if (i != 2 and i != 7) else '' for i in range(len(centers))]
        ...
        ax.text(
            0.98, 0.98, "sorted by mux-c and then mux-r",
            transform=ax.transAxes, ha='right', va='top',
            fontsize=18, fontweight='bold'
        )
        ...
        cbar_pad = 0.01
        cax = fig.add_axes([
            pos_right.x1 + cbar_pad,
            pos_right.y0,
            cbar_width,
            pos_right.height
        ])
        fig.colorbar(im2, cax=cax, label='correlation coefficient')
        plt.suptitle('Detector Pair-wise Correlation Coefficient (' + stagename + ' common mode removal)', fontsize=20, y=0.92)
        plt.savefig(savepath+'/detector_tod_pairwise_correlation'+stagename+'.png',dpi=500,bbox_inches='tight')
        plt.close()
    ...
    def plot_commonmode_template(self,templates,pixel,savepath,template_kind = 'mux'):
        ...
        plt.savefig(savepath+'/'+template_kind+'_common_mode_templates.png',dpi=500,bbox_inches='tight')

        plt.close()

    def normalize_tod(self,tod,strength=10,return_norms=False):
        """
        we want to put all detector tods on the same scale before computing the commmon mode template. 
        but especailly before calibration this is tricky as detectors can have very different counts 
        so we normalize it by the std of the middle 50% of the timestream scans. 

        strength: float: an aesthetics factor to scale the normalized tods.
        return_norms: bool: whether to return the normalization factors along with the normalized tods.
        """
        #tod, xf_full = self.build_full_xf_tod() 
        #tod = self.build_full_xf_tod()
        time = self.t
        #computes the std of the middle 50% of the timestream
        std = np.nanstd((tod)[:,len(time)//4:3*len(time)//4],axis=-1) 
        #normalize by that with an astetics factor of 10
        normed_tod = (tod)/(strength*std[:,None])
        if return_norms:
            return normed_tod, strength * std
        else:
            return normed_tod 
    
    #def unnormalize_tod(self,tod_norm,tod_norm_factor):
    #    """
    #    reverse the normalization process to get back to original units
    #    """
    #    return tod_norm * tod_norm_factor

    def build_full_xf_tod(self): 
        tod_goodxf = self.data
        xf = np.asarray(self.header['xf_coords'])
        if xf.ndim == 1:
            xf = np.vstack(xf)
        xf = xf.astype(int)
        xf_set = set(map(tuple, xf))
        idx_map = {tuple(pair): i for i, pair in enumerate(xf)}

        tod_full = np.full((960, tod_goodxf.shape[-1]), np.nan)
        #xf_full = np.array([[x, f] for f in range(60) for x in range(16)])

        for f in range(60):
            for x in range(16):
                i = idx_map.get((x, f), None)
                if i is not None:
                    tod_full[x + 16*f] = tod_goodxf[i]
        return tod_full #, xf_full

    def get_and_sort_xf(self,fx_sort=False,rc_sort=False): 
        xf = np.asarray(self.header['xf_coords'])
        if xf.ndim == 1:
            xf = np.vstack(xf)
        xf = xf.astype(int)
        ...
        xf_fxsort = xf[np.argsort(xf[:, 1])] if fx_sort else None
        xf_rcsort = sorted(xf,key=lambda xfi:coordinates.xf_to_muxcr(xfi[0],xfi[1])[0]*32+coordinates.xf_to_muxcr(xfi[0],xfi[1])[1]) if rc_sort else None

        if fx_sort and rc_sort: 
            return xf, xf_fxsort, xf_rcsort
        elif fx_sort:
            return xf,xf_fxsort
        elif rc_sort:
            return xf,xf_rcsort
