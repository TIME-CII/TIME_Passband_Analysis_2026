...
#def load_loadcurves_muxcr(foldername, calib, r_ser_override, partial=False):
	...
	#bias_raw = np.load(biasfilename, skiprows=1)
...
class LoadCurve:
	...
	def __init__(self, bias_dac, tes_dac, calib, fluxjmp=None, r_ser_override=None, partial=False, didi_noise_thresh=0.1):
		#print('bias_dac', bias_dac.shape)
		#print('tes_dac', tes_dac.shape)

		#assert(bias_dac.shape[1]) == (tes_dac.shape[0])
		assert((bias_dac.shape[0]) == tes_dac.shape[0])
	...
	@lru_cache()
	def bias_i(self):
		#print('bias', self.bias_dac)
		#print('calib tes bias', self.calib.CAL_TES_BIAS_I)
	...
	@lru_cache()
	def tes_i(self):
		#print('tes', self.tes_dac)
		#print('calib squid', self.calib.CAL_SQ1_FB_I)
	...
	def r_ser(self):
		if self.r_ser_override is not None:
			...
			print('slope', self.r_ser_fit_slope)
